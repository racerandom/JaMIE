...
train_tensors, train_deunk = extract_ner_from_conll('data/train_%s.txt' % CORPUS, tokenizer, lab2ix, device)
test_tensors, test_deunk = extract_ner_from_conll('data/test_%s.txt' % CORPUS, tokenizer, lab2ix, device)
...
save_bert_model(model, tokenizer, 'checkpoints/ner/')

output_file = 'outputs/ner_%s_ep%i_out.txt' % (CORPUS, NUM_EPOCHS)
